...
@Service("MaximumProtocol")
public class MaximumProtocolServiceImpl extends CommonPartProtocolServiceImpl {
    public MaximumProtocolServiceImpl(Config config, ActionService actionService, SessionService sessionService, LocalizationService localizationService, QuestionService questionService) {
        super(Session.State.FORWARD, config, actionService, sessionService, localizationService, questionService);
        this.transitions.put(Session.State.FORWARD, Session.State.MAXIMUM);
        this.transitions.put(Session.State.MAXIMUM, Session.State.DEFAULT);
    }

    @Override
    protected BotApiMethod<?> commit(Session session, ResourceBundle resourceBundle) {
        questionService.setMaximum(session.getChatId(), session.getPostId(), session.getMaximum());
